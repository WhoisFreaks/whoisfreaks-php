...
try {
    $result = $apiInstance->dbAsnWhois($date, $hostIndex, $variables);
    print_r($result);
} catch (Exception $e) {
...
```php
dbAsnWhoisStatus(): \WhoisFreaks\Model\SnapshotStatus
```
### URI(s):
- https://files.whoisfreaks.com 
...
try {
    $result = $apiInstance->dbAsnWhoisStatus($hostIndex, $variables);
    print_r($result);
} catch (Exception $e) {
