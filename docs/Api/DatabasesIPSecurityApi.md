...
```php
dbIpSecurity($apiKey, $date): \SplFileObject
```
### URI(s):
...
try {
    $result = $apiInstance->dbIpSecurity($apiKey, $date, $hostIndex, $variables);
    print_r($result);
} catch (Exception $e) {
...
```php
dbIpSecurityStatus($apiKey): \WhoisFreaks\Model\SnapshotStatus
```
### URI(s):
- https://files.whoisfreaks.com 
...
try {
    $result = $apiInstance->dbIpSecurityStatus($apiKey, $hostIndex, $variables);
    print_r($result);
} catch (Exception $e) {
