...
class SSLApi
{
     ...
     * @param  string $contentType The value for the Content-Type header. Check self::contentTypes['sslLookup'] to see the possible values for this operation
     *
    ...
    /**
     * Operation sslLookupWithHttpInfo
     *
     * SSL Certificate Lookup
     *
    ...
    public function sslLookupWithHttpInfo($domainName, $chain = false, $sslRaw = false, $format = 'json', string $contentType = self::contentTypes['sslLookup'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\WhoisFreaks\Model\SslResponse' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\WhoisFreaks\Model\SslResponse' !== 'string') {
                            try {
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\WhoisFreaks\Model\SslResponse', []),
    ...
    /**
     * Operation sslLookupAsyncWithHttpInfo
     *
     * SSL Certificate Lookup
     *
     * @param  string $domainName (required)
    ...
    public function sslLookupRequest($domainName, $chain = false, $sslRaw = false, $format = 'json', string $contentType = self::contentTypes['sslLookup'][0])
    {
        ...
        if ($domainName === null || (is_array($domainName) && count($domainName) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $domainName when calling sslLookup'
            );
        }
